...
export class RecordService extends GenericListService {
  ...
  updateRecordStatus(recordId: number, isApproved: boolean, isSchoolAdmin: boolean) {
    if (isSchoolAdmin) {
  ...
  getRecentEventsHaveRecords() {
    return this.http.get<CampusEvent[]>(`/records/recent-events/`);
  }
}
